...
# width is under it.
#
# Idempotent: re-running replaces the previously generated region. It also
# replaces a region previously generated by gen_footstep_scene.py, so the two
# scripts can be used interchangeably on the same scene XML.
#
# Usage:
...
# regions written by the older generators; replaced if found so both scripts
# can own the same scene XML
OLD_MARKS = [
    ("<!-- BEGIN footstep geoms (auto-generated by gen_footstep_scene.py) -->",
     "<!-- END footstep geoms -->"),
...
# ---------------------------------------------------------------------------
# XML injection (idempotent, also replaces gen_footstep_scene.py regions)
# ---------------------------------------------------------------------------
...
def inject(xml_text, block):
    for begin, end in [(BEGIN_MARK, END_MARK)] + OLD_MARKS:
